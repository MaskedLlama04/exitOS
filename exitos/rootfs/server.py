...
def start_communities_discovery_subscriber():
    ...
    def run_discovery():
        global _cached_communities, _cached_communities_source_host
        import random
        _first_reception = True  # Setejem a False un cop rebuda la llista per primera vegada
        while True:
            try:
                manager_ip = _get_addon_manager_ip()
                mqtt_port = 1883
                # Intentem llegir de la configuració guardada si hi ha
                try:
                    user_data = get_user_configuration_data()
                ...
                # ID únic per evitar que el broker desconnecti una sessió anterior amb el mateix ID
                unique_id = f"exitos_discovery_{random.randint(10000, 99999)}"
                try:
                    from paho.mqtt.enums import CallbackAPIVersion
                    disc_client = mqtt.Client(CallbackAPIVersion.VERSION2, client_id=unique_id)
                except (ImportError, AttributeError):
                    disc_client = mqtt.Client(client_id=unique_id)
                ...
                def _on_disc_connect(client, userdata, flags, reason_code, properties=None):
                    ...
                    if reason_code == 0:
                        ...
                        client.subscribe("exitos/communities/list")
                        # Només mostrem el missatge INFO la primera vegada; després queda en DEBUG
                        if _cached_communities is None:
                            logger.info(f"[DESCOBERTA] Subscrit a 'exitos/communities/list' via {manager_ip}:{mqtt_port}")
                        else:
                            logger.debug(f"[DESCOBERTA] Re-subscrit a 'exitos/communities/list' via {manager_ip}:{mqtt_port}")

                def _on_disc_message(client, userdata, msg):
                    nonlocal _first_reception
                    global _cached_communities, _cached_communities_source_host
                    try:
                        payload = json.loads(msg.payload.decode())
                        if payload.get("status") == "ok" and "communities" in payload:
                            community_names = [c['name'] for c in payload["communities"]]
                            _cached_communities = payload["communities"]
                            _cached_communities_source_host = manager_ip
                            # Mostrem INFO només la primera vegada; les actualitzacions posteriors en DEBUG
                            if _first_reception:
                                logger.info(f"[DESCOBERTA] Comunitats rebudes: {community_names}")
                                _first_reception = False
                            else:
                                logger.debug(f"[DESCOBERTA] Llista comunitats actualitzada: {community_names}")
                    except Exception as e:
